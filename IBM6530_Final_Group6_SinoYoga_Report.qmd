...
This shows: 
  - It is the primary entry point 
  - It carries the full burden of engagement and conversion Other pages (e.g., /zh-cn, /about-us) have minimal traffic
This suggests: 
...
- **First-Touch Attribution**  
  Assigns full credit to the first interaction channel, capturing awareness and discovery.

- **Last-Touch Attribution**  
  Assigns full credit to the final interaction channel, representing conversion or decision-stage behavior.

- **Linear Attribution**  
  Distributes equal credit across all touchpoints, reflecting the full customer journey.
...
- **YouTube plays a critical awareness role**  
  First-touch attribution indicates that YouTube is a primary entry point for users, supporting initial discovery and brand exposure.

- **Direct traffic dominates final interactions**  
  Last-touch attribution shows that many users return directly, suggesting that users revisit the site after initial exposure rather than converting immediately.
...
**Expected impact:** Establishes the foundation for measurable funnel performance, enables future attribution modeling on actual conversions, and unlocks GA4's conversion-based reporting.  

### Address the High Bounce Rate  
...
- Clearer above-the-fold messaging that communicates Sino Yoga's value proposition within 3 seconds
...
**Expected impact:** Reducing bounce rate from 89% to a more typical 60–70% range would meaningfully increase the pool of users available for retention and conversion efforts.

### Activate UTM Tracking Across All Marketing Channels

**Evidence:** Direct traffic dominates the channel mix (157 of 222 sessions). In digital marketing, "Direct" frequently masks untagged referrals from social media, email, and video platforms. The brand's existing YouTube presence (~2,300 subscribers) is not visibly contributing to website traffic, which is most likely an attribution gap rather than a true absence of referrals.
...
**Expected impact:** Within weeks of implementation, the team should see Direct traffic decrease and channel-specific traffic increase, providing accurate attribution data for future analysis and dashboard reporting.

### Build Returning Visitor Pathways
...
- A weekly content calendar promoted on the website with clear publishing days
- Account creation that saves user preferences or progress
- Retargeting through email or social channels to drive return visits

**Expected impact:** Increases lifetime value per acquired user and creates a pool of returning visitors that future conversion efforts can target more cost-effectively.

### Develop a Multi-Page Content Journey
...
- Embed featured YouTube videos directly on the website to deepen on-site engagement
- Create themed landing pages for major content categories (e.g., Five-Element Meridian Yoga, beginner classes)
- Implement related-content suggestions at the bottom of each page

**Expected impact:** Increases pages per session beyond the current 1.22 average, builds a richer behavioral dataset for analysis, and creates more conversion opportunities.

### Localize for the Audience the Site Already Has
...
- Continue investing in U.S. growth, but treat the existing Singapore audience as a near-term monetization opportunity rather than a secondary market

**Expected impact:** Captures revenue from the current audience while continuing to build the U.S. market, rather than under-serving existing users in pursuit of future ones.

## Limitations

### Data Pipeline Limitations

- **Limited reporting window.** The analysis covers approximately four weeks of data (April 7 – May 3, 2026). Patterns observed should be treated as a snapshot rather than established trends.

- **GA4 daily export still maturing.** The BigQuery export was recently linked to GA4, and the daily `events_*` tables that power the analytics mart are still populating. Future analyses on a longer time series will provide more robust findings.

- **Mart-to-dashboard integration in progress.** Some descriptive insights in this report are sourced from GA4's reporting interface rather than directly from the BigQuery mart. The mart documented in Section 2 is ready to power future dashboard iterations as the data pipeline matures.
...
- **Single-touchpoint dominance.** The current data is dominated by single-channel user paths (mostly Direct), which limits the explanatory power of multi-touch attribution models. As cross-channel pathways are activated through UTM tagging, attribution analysis will become more meaningful.
...
- **Website-only analysis.** This report focuses on website behavior captured by GA4 and BigQuery. YouTube channel performance, social media engagement, and offline conversion data are out of scope for this engagement.

- **No revenue or transactional data.** Sino Yoga's website does not currently process transactions, so revenue attribution and customer lifetime value calculations are not possible at this time.
